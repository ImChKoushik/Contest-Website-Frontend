...
export default function SignInForm() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 flex justify-center items-center transition-colors duration-300">
      <div className="w-full max-w-[1000px] bg-[var(--card-bg)] rounded-[32px] shadow-[var(--card-shadow)] flex flex-col md:flex-row overflow-hidden border border-[var(--border-primary)] transition-all">
        ...
        <div className="md:w-1/2 p-10 lg:p-14 flex flex-col justify-center bg-[var(--card-bg)] transition-colors">
          <div className="mb-8">
            <h2 className="text-[32px] font-bold text-[var(--text-primary)] mb-2 leading-tight transition-colors">Welcome back</h2>
            <p className="text-[var(--text-secondary)] text-[15px] transition-colors">Enter your credentials to access your academy dashboard.</p>
          </div>
          ...
          <Form onSubmit={handleSubmit} className="!p-0 !shadow-none !rounded-none !max-w-none w-full !bg-transparent">
            ...
            <div className="relative mb-0">
              <div className="flex justify-between items-center mb-2">
                <label className="text-sm font-semibold text-[var(--text-primary)] transition-colors">Email Address</label>
                {formData.email && (
                  <button
                    type="button"
                    onClick={handleResendVerification}
                    disabled={isResending}
                    className="text-xs font-semibold text-[var(--accent-green)] hover:underline bg-transparent border-none p-0 cursor-pointer disabled:opacity-50 transition-colors"
                  >
            ...
            <div className="relative mb-2">
              <div className="flex justify-between items-center mb-2">
                <label className="text-sm font-semibold text-[var(--text-primary)] transition-colors">Password</label>
                <a href="#" onClick={(e) => { e.preventDefault(); navigate('/forgot-password'); }} className="text-xs font-semibold text-[var(--accent-green)] hover:underline transition-colors">Forgot Password?</a>
              </div>
            ...
            <div className="flex items-center mb-8">
              <input
                ...
                className="w-4 h-4 rounded border-[var(--border-primary)] text-[var(--accent-green)] focus:ring-[var(--accent-green)] mt-0.5 bg-[var(--input-bg)] transition-colors"
              />
              <label htmlFor="remember" className="ml-3 text-[13px] text-[var(--text-secondary)] font-medium cursor-pointer transition-colors">Remember me for 30 days</label>
            </div>
            ...
            <div className="flex items-center mt-7 mb-7">
              ...
              <div className="px-5 text-[11px] font-bold text-[var(--text-secondary)] uppercase tracking-widest leading-none opacity-50 transition-colors">Or continue with</div>
              <div className="flex-1 border-t border-[var(--border-primary)]"></div>
            </div>

            <div className="flex gap-4 mb-8">
              <button type="button" className="flex-1 flex items-center justify-center gap-2.5 py-3 border border-[var(--border-primary)] rounded-xl hover:bg-[var(--bg-secondary)] transition-colors font-semibold text-[var(--text-primary)] text-sm shadow-sm group bg-[var(--input-bg)]">
          ...
          <p className="text-center text-[14px] text-[var(--text-secondary)] font-medium mt-7 transition-colors">
            Don't have an account? <Link to="/signup" className="text-[var(--accent-green)] font-bold hover:underline transition-colors hover:brightness-110">Sign Up</Link>
          </p>
        </div>
