      ...
      <div className="max-w-7xl mx-auto px-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-[var(--text-secondary)] font-bold uppercase tracking-widest text-xs">Loading {displayName} Challenges...</p>
          </div>
        ) : error ? (
          <div className="bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 p-8 rounded-[32px] text-center border border-red-100 dark:border-red-900/30 font-medium max-w-2xl mx-auto">
            <p className="mb-2">Failed to load contests. Please try again later.</p>
          </div>
        ) : filteredContests.length === 0 ? (
          <div className="bg-[var(--card-bg)] text-[var(--text-secondary)] p-20 rounded-[40px] text-center border border-[var(--border-primary)] font-medium flex flex-col items-center gap-6 max-w-3xl mx-auto shadow-sm transition-colors duration-300">
            <div className="w-20 h-20 bg-[var(--bg-primary)] rounded-full flex items-center justify-center mb-2">
            ...
            <p className="text-xl text-[var(--text-primary)] font-black uppercase tracking-tight">No active contests in {displayName}</p>
            <p className="text-sm max-w-sm">We're currently preparing new challenges for this category. Stay tuned!</p>
            <button
              onClick={() => navigate('/contests')}
              className="text-[#8cc63f] font-black uppercase tracking-widest text-[11px] hover:underline transition-colors mt-2"
            >
              View All Categories
            </button>
          </div>
