...
export default function TeamSelectionModal({ isOpen, onClose, contestId, contestTitle, projectType, onSuccess }) {
  const { createTeam, requestJoin, viewAllTeams, loading } = useTeam();
  ...
  const handleJoinRequest = async (teamId) => {
    const { success } = await requestJoin(teamId);
    if (success) {
      onSuccess();
      onClose();
    }
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1.5">
               ...
               <p className="text-[11px] text-[#8cc63f] font-black uppercase tracking-widest">
                {projectType === 'Individual' ? 'Solo Participation' : 'Team Challenge'}
              </p>
            </div>
          </div>
